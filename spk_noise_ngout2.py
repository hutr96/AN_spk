...
class GAN(object):
	...
	def creat_model(self):
		...
		loss_d_noise_ng= tf.reduce_mean(-tf.reduce_sum(self.labs_noise * tf.log((1-self.D1_noise) + 1e-10), 1), 0)
		...
		with tf.Session() as session:
			...
			for epoch in range(num_GAN):
				...
				for idx in range(0, batch_idxs):
					...
					tt = np.random.rand()
					if tt<0.5:
						loss_d, _ = session.run([self.loss_d, self.opt_d], {self.G_input: data, self.labs_spk: lab_spk, self.labs_noise: lab_noise})
					...
					self.logger.info('step ' + str(epoch)+' Batch ' + str(idx) + 'loss_d ' + str(loss_d) + ' loss_g1 ' + str(loss_g1) + ' loss_g2 ' + str(loss_g2) + ' loss_g3 ' + str(loss_g3))
...
result_path='/work/aaudeeplearning/hongyu/GAN_enh/result'
extname='spk_noise_ngout2'
...
mymodel.creat_model()
print('train 3')
mymodel.train_model('scp/rand_all.scp',0,30)
print('end')


		  
inpath = '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/ubm/mfcc/ubm_clean'
outpath = '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/ubm/GAN_' + extname
scp_UBM='scp/UBM.scp'
...
inpath = '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/train_spk/mfcc/train_spk_clean'
outpath= '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/train_spk/GAN_'+extname + '/train_spk_clean'
...
for noise in noises:
	for snr in snrs:
		inpath = '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/train_spk/mfcc/train_spk_'+noise+'/SNR_'+snr
		outpath= '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/train_spk/GAN_'+extname + '/train_spk_' + noise + '/SNR_'+ snr
		mymodel.test_model(scp_file=scp_train_spk, model_name=my_model_name, savepath_in=inpath,savepath_out=outpath)
...
#test clean
inpath = '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/test_spk/mfcc/test_spk_clean'
outpath= '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/test_spk/GAN_'+extname + '/test_spk_clean'
...
for noise in noises:
	for snr in snrs:
		inpath = '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/test_spk/mfcc/test_spk_'+noise+'/SNR_'+snr
		outpath= '/gpfs/gss1/work/aaudeeplearning/hongyu/GAN_enh/test_spk/GAN_'+extname + '/test_spk_' + noise + '/SNR_'+ snr
		mymodel.test_model(scp_file=scp_test_spk, model_name=my_model_name, savepath_in=inpath,savepath_out=outpath)
