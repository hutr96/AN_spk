...
class GAN(object):
	...
	def creat_model(self):
		...
		self.loss_d=loss_d_spk
		self.loss_g=loss_d_spk
		...
		with tf.Session() as session:
			...
			for epoch in range(num_GAN):
				...
				for idx in range(0, batch_idxs):
					...
					pretrain_loss, _ = session.run([self.pre_loss, self.pre_opt],{self.pre_input: data, self.pre_labs_noise: lab_noise, self.pre_labs_spk: lab_spk})
					...
					self.logger.info('step ' + str(epoch)+' Batch ' + str(idx) + 'pretrain_loss ' + str(pretrain_loss) )
